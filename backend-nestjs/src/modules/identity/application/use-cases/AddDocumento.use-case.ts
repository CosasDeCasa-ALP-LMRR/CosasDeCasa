...
export class AddDocumentoUseCase {
  ...
  async execute(
  ...
  ): Promise<{ documento?: Documento; estadoVerificacion: string; mensajeValidacion?: string }> {
    ...
    if ((tipo === 'INE' || tipo === 'CEDULA') && consentimientoIA) {
      ...
      const ocrResult = await this.ocrAdapter.extractIdentityData(fileBuffer, mimeType);
      ...
      if (!ocrResult.valido) {
        ...
        mensajeValidacion = ocrResult.motivoRechazo || 'El documento no se pudo leer automáticamente. Pasó a revisión manual.';
        ...
        const nameMatch = nameTokens.length > 0 && nameTokens.every(token => dbNombre.includes(token));
        ...
        if (curpMatch && nameMatch) {
          ...
          nuevoEstadoVerificacion = 'PENDIENTE';
          mensajeValidacion = 'Los datos no coinciden exactamente. Un auditor revisará el documento.';
          ...
          if (!usuario.curp && ocrResult.curp) {
             await this.usuarioRepository.update(usuario.id, { curp: ocrResult.curp.toUpperCase() });
          }
        }
      }
